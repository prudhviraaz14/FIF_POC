...
<!--
	XSLT file for creating a FIF request for a Business DSL Profi contract.
...
<xsl:stylesheet exclude-result-prefixes="dateutils" version="1.0"
	...
	<xsl:template match="request-params">
		...
		<xsl:variable name="contractName">Business DSL profi</xsl:variable>
		...
		<xsl:element name="CcmFifCreateContactCmd">
			...
			<xsl:element name="CcmFifCreateContactInCont">
				...
				<xsl:element name="contact_type_rd">ADD_BUSINESS_DSL</xsl:element>
				<xsl:element name="short_description">
					<xsl:text>Anlage DSL Profi Vertrag.</xsl:text>
				</xsl:element>
		...
		<xsl:element name="CcmFifCreateContactCmd">
			...
			<xsl:element name="CcmFifCreateContactInCont">
				...
				<xsl:element name="contact_type_rd">ADD_BUSINESS_DSL</xsl:element>
				<xsl:element name="short_description">
					<xsl:text>Anlage DSL Profi PS.</xsl:text>
				</xsl:element>
				<xsl:element name="long_description_text">
					<xsl:text>TransactionID: </xsl:text>
					<xsl:value-of select="request-param[@name='transactionID']"/>
					<xsl:text>&#xA;Business DSL ProfiProduct subscription has been added on:</xsl:text>
					<xsl:value-of select="request-param[@name='desiredDate']"/>
					<xsl:text>&#xA;Username:</xsl:text>
		...
		<xsl:if test="request-param[@name='requestListId'] != ''">
			<xsl:element name="CcmFifCreateExternalNotificationCmd">
				...
				<xsl:element name="CcmFifCreateExternalNotificationInCont">
					...
					<xsl:element name="notification_action_name">CreateBusinessDSLProfiContract</xsl:element>
					<xsl:element name="target_system">FIF</xsl:element>
					<xsl:element name="parameter_value_list">
						<xsl:element name="CcmFifParameterValueCont">
							<xsl:element name="parameter_name">DSL_PROFI_SERVICE_SUBSCRIPTION_ID</xsl:element>
