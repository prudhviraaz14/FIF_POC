...
<xsl:stylesheet exclude-result-prefixes="dateutils" version="1.0"
  ...
  <xsl:template match="request-params">
    ...
    <xsl:element name="Command_List">
      ...
      <!-- Ensure that the paramter salesOrganisationNumber is set if productCommitmentNumber isn't provided-->
      <xsl:if test="(request-param[@name='productCommitmentNumber'] = '')      
        and (request-param[@name='salesOrganisationNumber'] = '')">
        <xsl:element name="CcmFifRaiseErrorCmd">
          <xsl:element name="command_id">error_1</xsl:element>
          <xsl:element name="CcmFifRaiseErrorInCont">
            <xsl:element name="error_text">Mandatory parameter salesOrganisationNumber missed from Request. </xsl:element>
          </xsl:element>
        </xsl:element>
      </xsl:if> 
  
  
      <!-- Ensure that the paramter tariff is set if productCommitmentNumber isn't provided-->
      <xsl:if test="(request-param[@name='productCommitmentNumber'] = '')      
        and (request-param[@name='tariff'] = '')">
        <xsl:element name="CcmFifRaiseErrorCmd">
          <xsl:element name="command_id">error_2</xsl:element>
          <xsl:element name="CcmFifRaiseErrorInCont">
            <xsl:element name="error_text">Mandatory parameter tariff missed from Request. </xsl:element>
          </xsl:element>
        </xsl:element>
      </xsl:if> 

      <xsl:if test="(request-param[@name='accessNumber1'] != '')                              
        and (request-param[@name='accessNumberCount'] != '0') ">
        <xsl:element name="CcmFifRaiseErrorCmd">
          <xsl:element name="command_id">error_3</xsl:element>
          <xsl:element name="CcmFifRaiseErrorInCont">
            <xsl:element name="error_text">accessNumberCount must be set to '0' if old access numbers are taken over!</xsl:element>
          </xsl:element>
        </xsl:element>
      </xsl:if>
      ...
      <xsl:element name="CcmFifAddServiceSubsCmd">
        ...
        <xsl:element name="CcmFifAddServiceSubsInCont">
          ...
          <xsl:element name="service_characteristic_list">
            ...
            <!-- old Service Subscription Id -->
            <xsl:element name="CcmFifConfiguredValueCont">
              <xsl:element name="service_char_code">VI323</xsl:element>
              <xsl:element name="data_type">STRING</xsl:element>
              <xsl:element name="configured_value">
                <xsl:value-of select="request-param[@name='oldServiceSubscriptionId']"/>
              </xsl:element>
            </xsl:element>  
          </xsl:element>
        </xsl:element>
          ...
          <xsl:element name="parameter_value_list">
            ...
            <xsl:if test="request-param[@name='productCommitmentNumber']=''">
              ...
              <xsl:element name="CcmFifParameterValueCont">
                ...
                <xsl:if test="request-param[@name='boardSignDate'] != ''"> 
                  <xsl:element name="parameter_value"> 
                    <xsl:value-of select="request-param[@name='boardSignDate']"/>
                  </xsl:element>
                </xsl:if> 
