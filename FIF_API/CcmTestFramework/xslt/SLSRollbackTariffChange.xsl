      ...
      <xsl:element name="CcmFifRollbackApplyNewPriceStructCmd">
        ...
        <xsl:element name="CcmFifRollbackApplyNewPriceStructInCont">
          ...
          <xsl:element name="supported_object_type_rd">
          	<xsl:if test="(request-param[@name='SUPPORTED_OBJECT_TYPE_RD'] = 'O')">ORDERFORM</xsl:if>
          	<xsl:if test="(request-param[@name='SUPPORTED_OBJECT_TYPE_RD'] = 'S')">SERVDLVRY</xsl:if>
          </xsl:element>
        </xsl:element>
      </xsl:element>
