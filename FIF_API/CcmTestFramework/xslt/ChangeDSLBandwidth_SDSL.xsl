...
-->
  ...
  <xsl:element name="CcmFifCommandList">
    ...
    <xsl:element name="action_name">
      <xsl:value-of select="concat($TopAction, '_SDSL')"/>
    </xsl:element>    
    ...
    <xsl:element name="Command_List">
      ...
      <!-- if changeDSLBandwidthAndTariff error out  -->
      <xsl:if test="request-param[@name='changeDSLBandwidthAndTariff'] = 'Y'">
        <xsl:element name="CcmFifRaiseErrorCmd">
          ...
          <xsl:element name="CcmFifRaiseErrorInCont">
            <xsl:element name="error_text">Transaction changeDSLBandwidthAndTariff does not support SDSL!</xsl:element>
          </xsl:element>
        </xsl:element>
      </xsl:if>
      ...
      <xsl:if test="($newDSLBandwidth!= 'DSL 2000')
        and ($newDSLBandwidth!= 'DSL 4000')">
        <xsl:element name="CcmFifRaiseErrorCmd">
          <xsl:element name="command_id">new_dsl_bandwidth_error</xsl:element>
          <xsl:element name="CcmFifRaiseErrorInCont">
            <xsl:element name="error_text">Invalid new DSL bandwidth value passed in for SDSL. Passed in value: <xsl:value-of select="$newDSLBandwidth"/>. Allowed values: DSL 2000,and DSL 4000.</xsl:element>
          </xsl:element>
        </xsl:element>
      </xsl:if>  
      ...
      <!-- find an open customer order for the main access service  for SDSL-->
      ...
      <!-- Reconfigure the main access service on desired date SDSL scenario if a customer order found -->
      <xsl:element name="CcmFifReconfigServiceCmd">
        <xsl:element name="command_id">reconf_serv_1</xsl:element>
        <xsl:element name="CcmFifReconfigServiceInCont">
          <xsl:element name="service_subscription_ref">
      ...
      <!-- Find the DSL bandwidth service for SDSL-->
      ...
      <xsl:if test="$newDSLBandwidth != $oldDSLBandwidth">
        ...
        <!-- Only terminate if the DSL bandwidth is changing for SDSL-->          
      ...
      <xsl:element name="CcmFifCreateExternalNotificationCmd">
        ...
        <xsl:element name="CcmFifCreateExternalNotificationInCont">
          ...
          <xsl:element name="parameter_value_list">
            ...
            <xsl:element name="CcmFifParameterValueCont">
              <xsl:element name="parameter_name">SDSL_SERVICE_SUBSCRIPTION_ID</xsl:element>
              <xsl:element name="parameter_value_ref">
                <xsl:element name="command_id">find_service_1</xsl:element>
                <xsl:element name="field_name">service_subscription_id</xsl:element>
              </xsl:element>
