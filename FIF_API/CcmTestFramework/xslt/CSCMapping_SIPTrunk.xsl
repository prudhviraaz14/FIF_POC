<xsl:variable name="dataType">
  <xsl:choose>
    ...
    <xsl:when test="request-param[@name='parameterName'] = 'gatewayPin'">STRING</xsl:when>
    <xsl:when test="request-param[@name='parameterName'] = 'oneGroupID'">TECH_SERVICE_ID</xsl:when>
    <xsl:when test="request-param[@name='parameterName'] = 'sipTrunkVoiceChannels'">INTEGER</xsl:when>
    <xsl:when test="request-param[@name='parameterName'] = 'sipTrunkVoiceChannelsRange'">STRING</xsl:when>
    <xsl:when test="request-param[@name='parameterName'] = 'isPbxSipReady'">STRING</xsl:when>
    <xsl:when test="request-param[@name='parameterName'] = 'initialContractStartDate'">STRING</xsl:when>
    <xsl:when test="request-param[@name='parameterName'] = 'isPbxSipReadyIndicator'">BOOLEAN</xsl:when>
...
<xsl:variable name="serviceCharCode">
  <xsl:choose>
    ...
    <xsl:when test="request-param[@name='parameterName'] = 'gatewayPin'">VI068</xsl:when>
    <xsl:when test="request-param[@name='parameterName'] = 'oneGroupID'">VI086</xsl:when>
    <xsl:when test="$phoneSystemType = 'S2M' and request-param[@name='parameterName'] = 'sipTrunkVoiceChannels'">VI064</xsl:when>
    <xsl:when test="$phoneSystemType != 'S2M' and request-param[@name='parameterName'] = 'sipTrunkVoiceChannels'">VI095</xsl:when>
    <xsl:when test="$phoneSystemType != 'S2M' and request-param[@name='parameterName'] = 'sipTrunkVoiceChannelsRange'">VI096</xsl:when>
    <xsl:when test="$phoneSystemType = 'S2M' and request-param[@name='parameterName'] = 'sipTrunkVoiceChannelsRange'">VI067</xsl:when>
    <xsl:when test="request-param[@name='parameterName'] = 'isPbxSipReady'">VI069</xsl:when>
    <xsl:when test="request-param[@name='parameterName'] = 'initialContractStartDate'">V0200</xsl:when>
    <xsl:when test="request-param[@name='parameterName'] = 'phoneSystemMasterSlave'">VI089</xsl:when>
    <xsl:when test="request-param[@name='parameterName'] = 'phoneSystemType'">VI090</xsl:when>
    <xsl:when test="request-param[@name='parameterName'] = 'isPbxSipReadyIndicator'">VI091</xsl:when>
    <xsl:when test="request-param[@name='parameterName'] = 'phoneSystemModel'">VI092</xsl:when>
    <xsl:when test="request-param[@name='parameterName'] = 'processingType'">VI097</xsl:when>
...
<xsl:variable name="value">
  <xsl:choose>
    ...
    <xsl:when test="request-param[@name='parameterName'] = 'isPbxSipReady'">
      <xsl:choose>
        <xsl:when test="$valueToUse = 'true'">True</xsl:when>
        <xsl:otherwise>False</xsl:otherwise>
      </xsl:choose>
    </xsl:when>
    <xsl:when test="request-param[@name='parameterName'] = 'isPbxSipReadyIndicator'">
      <xsl:choose>
        <xsl:when test="$valueToUse = 'true'">Y</xsl:when>
        <xsl:otherwise>N</xsl:otherwise>
      </xsl:choose>
    </xsl:when>
    <xsl:when test="request-param[@name='parameterName'] = 'initialContractStartDate'">
      <xsl:choose>
        <xsl:when test="$valueToUse = 'today'">
          <xsl:value-of select="dateutils:getCurrentDate()"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="dateutils:SOM2CCBDate($valueToUse)"/>
        </xsl:otherwise>
      </xsl:choose>
