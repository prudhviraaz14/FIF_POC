...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="request-params">
    ...
    <xsl:element name="Command_List">
      ...
      <xsl:variable name="ReasonRd">PRODCHANGE_PREM</xsl:variable> 
      ...
      <xsl:if
        test="request-param[@name='productCode'] != $ISDNProductCode
         ...
         and request-param[@name='productCode'] != BitStreamVoIPProductCode">
        <xsl:element name="CcmFifRaiseErrorCmd">
          <xsl:element name="command_id">upgrade_error_1</xsl:element>
          <xsl:element name="CcmFifRaiseErrorInCont">
            <xsl:element name="error_text">Upgrade to Premium is only allowed for V0002, VI1203 and I1204!</xsl:element>
          </xsl:element>
        </xsl:element>
      </xsl:if> 

      <xsl:if test="request-param[@name='numberOfNewAccessNumbers'] &gt; '9' ">
        <xsl:element name="CcmFifRaiseErrorCmd">
          <xsl:element name="command_id">upgrade_error_3</xsl:element>
          <xsl:element name="CcmFifRaiseErrorInCont">
            <xsl:element name="error_text">numberOfNewAccessNumbers can not be greater than 9!</xsl:element>
          </xsl:element>
        </xsl:element>
      </xsl:if>
      ...
      <xsl:if test="request-param[@name='accessNumber1'] = ''                              
        ...
        <xsl:element name="CcmFifRaiseErrorCmd">
          <xsl:element name="command_id">upgrade_error_4</xsl:element>
          <xsl:element name="CcmFifRaiseErrorInCont">
      ...
      <xsl:if test="request-param[@name='internetFunctionID'] != ''">         
        <xsl:element name="CcmFifConcatStringsCmd">
          <xsl:element name="command_id">internetFunctionID</xsl:element>
      ...
      <xsl:if test="request-param[@name='voiceFunctionID'] != ''">         
        <xsl:element name="CcmFifConcatStringsCmd">
          <xsl:element name="command_id">voiceFunctionID</xsl:element>
          <xsl:element name="CcmFifConcatStringsInCont">
            <xsl:element name="input_string_list">
              <xsl:element name="CcmFifPassingValueCont">
                <xsl:element name="value">
